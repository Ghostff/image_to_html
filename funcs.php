...
class img_to_htm
{
    ...
    private static function align($image_data, $holder, $child, $width, $height, $styl_type)
    {
        ...
        foreach ($holder as $key => $attr) {
            ...
            foreach ($attr as $attribute => $value) {
                //geting id if attribute has id
                if ($attribute == 'id') {
                    $new_id = '#'.$value;
                }
                //geting class if attribute has class
        ...
        if ($styl_type == 'IL' || $styl_type == 'inline') {
            ...
            $HTML = '<style>'.$p_tag.'{width:'.$img_width*$width.'px;height:'.$img_height*$height;
            $HTML .= 'px;}'.$p_tag.' '.$c_tag.'{width:'.$width.'px;height:'.$height.'px;float:left;}</style>';
            $HTML .= $parent;
        }
    ...
    public static function render($img_name, $htm, $width = null, $height = null, $type = null, $color = null, $styl_type = null)
    {
        ...
        set_time_limit(0);
        if ($type == 'string') {
            return htmlspecialchars(self::align(self::ttl_pxls($img_name, $color),
                                    $htm['parent'], $htm['child'],  $width, $height, $styl_type));
        }
        else { 
            return self::align(self::ttl_pxls($img_name, $color), $htm['parent'],
                                    $htm['child'],  $width, $height, $styl_type);
        }
    }    
}
