...
Were @1param is the image to find
     @2param is the folder/path to search for image that macthes param 1
...
#1 match (lesser time) returns 1 image that matched
    echo find_img::matches('file.png', 'find', 1);
    
#2 match (2x slower) returns 2 image that matched
    var_dump( find_img::matches('file.png', 'find', 2));
    
#3 match (3x slower) returns 3 image that matched
     var_dump( find_img::matches('file.png', 'find', 3));
     
#...

#all match (more time) returns all image that matched
     var_dump( find_img::matches('file.png', 'find', 'all'));
```